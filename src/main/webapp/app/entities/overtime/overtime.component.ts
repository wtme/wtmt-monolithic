...
export class OvertimeComponent implements OnInit, OnDestroy {
  ...
  trackId(index: number, item: IOvertime) {
    return item.id;
  }

  registerChangeInOvertimes() {
    this.eventSubscriber = this.eventManager.subscribe('overtimeListModification', response => this.loadAll());
  }
  ...
  protected paginateOvertimes(data: IOvertime[], headers: HttpHeaders) {
